...
for i=1:iter_times
    DX(:,i)=Para_x(N+1,1:3)';
    Para_x(1:N,1:3)=x2(:,i+1:i+N)';
    Para_x(1:N,4:5)=v2(:,i:i+N-1)';
    Para_x(N+1+obstacle_num_ex+1,1:2)=min_distance_obs(4, 3.7, 0.5, 100, Para_x(N+1,1), Para_x(N+1,2))';
    Para_x(N+1+obstacle_num_ex+2,1:2)=min_distance_obs(5.7, 3.7, 1.2, 100, Para_x(N+1,1), Para_x(N+1,2))';
    tic;
...
figure
hold on;
plot(1:iter_times,DV2(1,:),'g');
plot(1:iter_times,v2(1,1:iter_times),'g');
title('智能体2的MPC线速度图');
xlabel('s');
ylabel('m/s');
