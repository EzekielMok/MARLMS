...
for i=1:iter_times
    DX(:,i)=Para_x(N+1,1:3)';
    Para_x(1:N,1:3)=x3(:,i+1:i+N)';
    Para_x(1:N,4:5)=v3(:,i:i+N-1)';
    Para_x(N+1+obstacle_num_ex+1,1:2)=min_distance_obs(4, -2.9, 0.5, 100, Para_x(N+1,1), Para_x(N+1,2))';
    Para_x(N+1+obstacle_num_ex+2,1:2)=min_distance_obs2(6.5, -2.9, 2, 0.6, 100, Para_x(N+1,1), Para_x(N+1,2))';
    tic;
...
figure
hold on;
plot(1:iter_times,DV3(1,:),'b');
plot(1:iter_times,v3(1,1:iter_times),'b');
title('智能体3的MPC线速度图');
xlabel('s');
ylabel('m/s');
